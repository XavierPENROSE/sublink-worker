...
app.get('/shorten-v2', async (c) => {
    try {
        ...
        if (!queryString) {
            return c.text('URL must contain a query string', 400);
        }
...
/**
 * Build the public URL for a generated subscription endpoint.
 *
 * The Worker is deployed under BASE_PATH (for example
 * /sublink-worker), while createApp() internally sees routes such as
 * /clash and /singbox.
 *
 * Keeping this logic centralized prevents short-link redirects and
 * /resolve from accidentally generating root-level URLs such as
 * /clash.
 */
const buildPublicEndpointUrl = (origin, prefix, queryString = '') => {
    const normalizedBasePath =
        typeof BASE_PATH === 'string'
            ? BASE_PATH.trim().replace(/^\/+|\/+$/g, '')
            : '';
    ...
    if (normalizedPrefix) {
        pathParts.push(normalizedPrefix);
    }

    return `${origin}/${pathParts.join('/')}${normalizedQuery}`;
};
...
const redirectHandler = (prefix) => async (c) => {
    try {
        ...
        if (!code || typeof code !== 'string') {
            return c.text('Invalid short code', 400);
        }
        ...
        const redirectUrl = buildPublicEndpointUrl(
            url.origin,
            prefix,
            originalParam
        );
...
app.get('/resolve', async (c) => {
    try {
        ...
        if (!shortUrl) {
            return c.text(t('missingUrl'), 400);
        }
         ...
         *
         *   /c/XXXXXX
         *   /sublink-worker/c/XXXXXX
         *
         * This is important because the public Worker is mounted
         * below BASE_PATH, while createApp() itself operates on
         * stripped internal paths.
         */
        ...
        const baseParts = normalizedBasePath
            ? normalizedBasePath.split('/').filter(Boolean)
            : [];

        if (
            baseParts.length > 0 &&
            pathParts.length >= baseParts.length + 2 &&
            baseParts.every((part, index) => pathParts[index] === part)
        ) {
            const offset = baseParts.length;
            prefix = pathParts[offset];
            shortCode = pathParts[offset + 1];
        } else if (pathParts.length >= 2) {
            prefix = pathParts[pathParts.length - 2];
        ...
        if (!originalParam) {
            return c.text(t('shortUrlNotFound'), 404);
        }
        ...
        const originalUrl = buildPublicEndpointUrl(
            urlObj.origin,
            mapping[prefix],
            originalParam
        );
